Feature: Policy-gated access
  ...
  So that new authorisation rules (per-repo access, etc.) land in one place

  Scenario: Default policy allows any authenticated caller
  ...
  Scenario: Swapping in a deny-all policy blocks authenticated callers
    Given the server is running with auth enabled
